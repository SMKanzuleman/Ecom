...
export const PlaceholderImage = ({ className = "", text = "No Image" }: Props) => {
    return (
        <div 
            className={`w-full aspect-square bg-gray-100 rounded-xl flex flex-col items-center justify-center text-gray-400 select-none ${className}`}
        >
            {/* Minimalist Shopping / Product Icon */}
            <svg 
                className="w-15 h-15 stroke-[1.2] transition-transform duration-300 group-hover:scale-110" 
                fill="none" 
                viewBox="0 0 24 24" 
                stroke="currentColor"
            >
